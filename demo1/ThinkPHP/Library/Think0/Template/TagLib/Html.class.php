...
class Html extends TagLib{
    ...
    public function _editor($tag,$content) {
        ...
        $width		=	!empty($tag['width'])?$tag['width']: '99%';
    ...
    public function _imgLink($tag) {
        ...
        if(empty($type)) {
       	...
       	$parseStr   = '<span class="'.$style.'" ><input title="'.$alt.'" type="'.$type.'" id="'.$id.'"  name="'.$name.'" onmouseover="this.style.filter=\'alpha(opacity=99)\'" onmouseout="this.style.filter=\'alpha(opacity=80)\'" onclick="'.$click.'" align="absmiddle" class="'.$name.' imgLink"></span>';
