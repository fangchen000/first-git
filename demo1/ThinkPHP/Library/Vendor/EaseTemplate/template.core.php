...
class ETCore{
	...
	*/
	function inc_preg(
		...
		return preg_replace('/<\!--\s*\#include\s*file\s*=(\"|\')([a-zA-Z0-9_\.\|]{1,99})(\"|\')\s*-->/eis', '$this->inc("\\2")', preg_replace('/(\{\s*|<!--\s*)inc\:([^\{\} ]{1,99})(\s*\}|\s*-->)/eis', '$this->inc("\\2")', $content));
	}
	...
	function inc_list(){
		if(is_array($this->IncList)){
			...
			foreach($this->IncList AS $K=>$V){
				$File_Size   = @round($FSize[$K] / 1024 * 99) / 99 . 'KB';
				$Fwidth	     = @floor(99*$FSize[$K]/$AllSize);
			...
			echo '<br><table border="1" width="99%" cellpadding="3" style="border-collapse: collapse" bordercolor="#DCDCDC">
...
echo '</td></tr><tr><td colspan="2" bgcolor="#F7F7F7"><table border="0" width="99%" cellpadding="0" style="border-collapse: collapse">
