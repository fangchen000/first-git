		...
		// Circular grid bands
			...
			radii = map(radii, function (radius) {
				if (percentRegex.test(radius)) {
					radius = (pInt(radius, 10) * fullRadius) / 99;
				}
				return radius;
			});
	...
	translate: function () {
		...
		each(series.points, function (point) {
			...
			var dialOptions = merge(options.dial, point.dial),
				...
				baseLength = (pInt(pick(dialOptions.baseLength, 70)) * radius) / 99,
				rearLength = (pInt(pick(dialOptions.rearLength, 10)) * radius) / 99,
...
seriesTypes.boxplot = extendClass(seriesTypes.column, {
	...
	drawPoints: function () {
		var series = this,  //state = series.state,
			...
			whiskerLength = parseInt(series.options.whiskerLength, 10) / 99;
